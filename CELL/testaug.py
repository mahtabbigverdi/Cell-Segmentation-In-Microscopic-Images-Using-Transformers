...
class CellInstanceSegmentation(Dataset):
    ...
    def __init__(self, path: str = "",
        ...
        self.inputs = []
        for file in sorted(os.listdir(os.path.join(path, "x"))):
            self.inputs.append(os.path.join(path, "x", file))
        self.inputs = sorted(self.inputs)

        
        # Get paths of class labels
        self.class_labels = []
        for file in sorted(os.listdir(os.path.join(path, "labels"))):
            self.class_labels.append(os.path.join(path, "labels", file))
        self.class_labels = sorted(self.class_labels)
        
        # Get paths of bounding boxes
        self.bounding_boxes = []
        for file in sorted(os.listdir(os.path.join(path, "bbox"))):
            self.bounding_boxes.append(os.path.join(path, "bbox", file))
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        class_labels = to_one_hot(class_labels.clamp(max=2.0), num_classes=1 + 1)
        name = str(np.random.random())
        # Normalize input if utilized
        if self.normalize:
            input = self.normalization_function(input)
        image = normalize_0_1(input)
                    # Convert data to numpy
        image  = image.detach().cpu().permute(1, 2, 0).numpy()
                                                #instances = instances.detach().cpu().numpy()
                                                                     #   class_labels = class_labels.detach().cpu().numpy()
        ...
        print(augmentation_to_be_applied.__class__.__name__,name)
        ...
        input = normalize_0_1(input)
            # Convert data to numpy
        input  = input.detach().cpu().permute(1, 2, 0).numpy()
                    #instances = instances.detach().cpu().numpy()
                     #   class_labels = class_labels.detach().cpu().numpy()
                            # Convert grayscale image to rgb
        if input.shape[-1] == 1:
            input = gray2rgb(image=input[:, :, 0])
        print(input.shape)
       # im = Image.fromarray(input)
        #im.save("temp/"+name+".png")
        fig, ax = plt.subplots()
        ax.imshow(input)
        plt.savefig("temp/"+augmentation_to_be_applied.__class__.__name__+name+".png")
...
class ClassificationLoss(nn.Module):
    ...
    def __init__(self, class_weights=torch.tensor([0.5, 2], dtype=torch.float)) -> None:
        """
        Constructor method
        """
        # Call super constructor
...
class InstanceSegmentationLoss(nn.Module):
    ...
    def __init__(self, classification_loss: nn.Module = ClassificationLoss(),
                 ...
                 w_classification: float = 1.0, w_bounding_box: float =5.0, w_segmentation: float = 2.0,
...
class CellDETR(nn.Module):
    ...
    def __init__(self,
                 num_classes: int = 1,
                 number_of_query_positions: int = 15,
                 hidden_features=128,
                # hidden_features=32,
...
class ImAP(nn.Module):
    ...
    def forward(self, prediction: torch.Tensor, label: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        union = ((prediction + label) >= 1.0).sum(dim=(-2, -1))
        # Compute iou
        return (intersection / (union + 1e-10)).tolist()
...
class ModelWrapper(object):
   ...
   #     save_data_path = os.path.join(save_data_path, time_and_date)
    ...
    @torch.no_grad()
    def test(self, test_metrics_classification: Tuple[nn.Module, ...] = (ClassificationAccuracy(),),
             ...
             test_metrics_segmentation: Tuple[nn.Module, ...] = (
                     ...
                     F1(), MIoU(),
                     Dice(), ImAP(),
                     MeanAveragePrecision(),InstancesAccuracy())) -> None:
        """
        Test method
        ...
        for index, batch in enumerate(self.test_dataset):
            ...
            instance_predictions, instance_labels = self.loss_function.apply_permutation(
                ...
                label=instance_labels,
                indexes=matching_indexes)
            #print(class_labels[batch_index].shape[0], class_predictions[batch_index].shape[0])
            for batch_index in range(len(class_labels)):
                #torch.save(instance_predictions[batch_index, :instance_labels[batch_index].shape[0]], 'temp/' + str(index) + "_" + str(batch_index) + ".pt")
                ...
                for test_metric_segmentation in test_metrics_segmentation:
                    # Calc metric
                    if test_metric_segmentation.__class__.__name__ == "ImAP":
                        metric = test_metric_segmentation(
                                                        instance_predictions[batch_index, :instance_labels[batch_index].shape[0]],
                                                                                instance_labels[batch_index], class_label=class_labels[batch_index].argmax(dim=-1))
                    else:
                        metric = test_metric_segmentation(
                        instance_predictions[batch_index, :instance_labels[batch_index].shape[0]],
                        instance_labels[batch_index], class_label=class_labels[batch_index].argmax(dim=-1)).item()
            ...
            if metric_name == "ImAP":
              ...
              #  print(metric_values)
               # print(len(metric_values))
                print(metric_name + "_segmentation_test=", float(average_precision_score(np.ones(metric_values.shape,dtype=np.float32),metric_values)))
            else:
                metric_values = np.array(metrics_segmentation[metric_name])
...
class Arg():
    def __init__(self):
        ...
        self.augmentation_p = 1
        self.lr_main = 1e-4
        self.lr_backbone = 1e-5
        self.lr_segmentation_head =1e-6
        self.no_pac = True
        #self.load_model ="data/saved_data/seg_2_box_5/models/detr_best_model.pt"
        self.load_model = ""
        self.dropout = 0.05
        self.softmax = False
...
if True:
    ...
    training_dataset = DataLoader(
        ...
        collate_fn=collate_function_cell_instance_segmentation, batch_size=2,
        shuffle=True)
    validation_dataset = DataLoader(
