...
class CellDETR(nn.Module):
    ...
    def __init__(self,
                 num_classes: int = 2,
                 number_of_query_positions: int = 28,
                 hidden_features=128,
                # hidden_features=32,
        ...
        for epoch in range(epochs):
            for input, instance_labels, bounding_box_labels, class_labels in self.training_dataset:
                ...
                if epoch < optimize_only_segmentation_head_after_epoch:
                    ...
                    scaler.scale((loss_classification + loss_bounding_box + loss_segmentation).half()).backward()
                   # Optimize detr
                    scaler.step(self.detr_optimizer)
                    #self.detr_optimizer.step()
                else:
                    # Perform backward pass to compute the gradients
                    #loss_segmentation.backward()
                    scaler.scale(loss_classification + loss_bounding_box + loss_segmentation).backward()
                    # Optimize detr
                    #self.detr_segmentation_optimizer.step()
                    scaler.step(self.detr_segmentation_optimizer)
                scaler.update()
...
class Arg():
    def __init__(self):
        ...
        self.no_pau = True
        self.only_train_segmentation_head_after_epoch = 150
